...
import { NumericFormat } from "react-number-format"; // Se importa NumericFormat para el formateo en tiempo real
...
export default function ExpenseForm() {
    ...
    // Estado para saber si el input de monto tiene el foco
    ...
    const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        // Formatear monto a dos decimales para el mensaje
        const formattedAmount = expense.amount.toFixed(2);

        // Muestra un mensaje de éxito en pantalla y en alerta
        alert(`Precio gasto agregado con éxito: ${formattedAmount}`);
        ...
        <div>
            <form className="space-y-5" onSubmit={handleSubmit}>
                ...
                <div className="flex flex-col gap-2">
                    ...
                    {/* Reemplazo del input de tipo number por NumberFormat para formatear en tiempo real */}
                    <NumericFormat
                        id="amount"
                        className="bg-slate-100 p-2"
                        placeholder="Añade la cantidad del gasto: ej. 300"
                        name="amount"
                        // Si el input está en foco y el monto es 0, se muestra como vacío
                        value={
                            isAmountFocused && expense.amount === 0
                                ? ""
                                : expense.amount
                        }
                        thousandSeparator={true}
                        decimalScale={0} //Aqui se decide cuantos decimales quieres mostrar ene ste casso seran 0
                        fixedDecimalScale={true}
                        allowNegative={false}
                        onFocus={() => setIsAmountFocused(true)}
                        onBlur={(e) => {
                            setIsAmountFocused(false);
                            // Si el usuario deja el campo vacío, se guarda 0
